...
ReadImportLookupTableFromFile(
    ...
    while (pd->OriginalFirstThunk) {
        ...
        PIMAGE_THUNK_DATA32 pIlt = (PIMAGE_THUNK_DATA32)(lpPe + Va2Ra(lpPe, pd->OriginalFirstThunk));
        PIMAGE_THUNK_DATA32 pIat = (PIMAGE_THUNK_DATA32)(lpPe + Va2Ra(lpPe, pd->FirstThunk));
...
ReadImportLookupTableFromProc(
    ...
    while (pd->OriginalFirstThunk) {
        ...
        PIMAGE_THUNK_DATA32 pIlt = (PIMAGE_THUNK_DATA32)(lpBuf + pd->OriginalFirstThunk);
        PIMAGE_THUNK_DATA32 pIat = (PIMAGE_THUNK_DATA32)(lpBuf + pd->FirstThunk);
    ...
    while (pd->OriginalFirstThunk) {
        ...
        while (pIlt->u1.AddressOfData and pIat->u1.AddressOfData) {
            ...
            if (strcmp(szFunctionName, szTargetName) == 0) {
                ...
                PIMAGE_THUNK_DATA32 pImageIat = (PIMAGE_THUNK_DATA32)(lpImage + ((LPBYTE)pIat - lpBuf));
...
//
//        PIMAGE_THUNK_DATA32 pIlt = (PIMAGE_THUNK_DATA32)(lpImage + pd->OriginalFirstThunk);
//        PIMAGE_THUNK_DATA32 pIat = (PIMAGE_THUNK_DATA32)(lpImage + pd->FirstThunk);
//
//        // Iterate through functions
//        while (pIlt->u1.AddressOfData and pIat->u1.AddressOfData) {
//            ULONGLONG lpFunctionAddr = pIat->u1.AddressOfData;
...
PIMAGE_EXPORT_DIRECTORY GetExportDirectoryTable(LPBYTE lpPe, PIMAGE_OPTIONAL_HEADER32 pOptHeader, LONG lpad = 0) {
...
//                    , std::map<DWORD, IMAGE_THUNK_DATA32>& mILT
//                    , std::map<DWORD, IMAGE_THUNK_DATA32>& mIAT
...
//                         i++, pIlt += sizeof(IMAGE_THUNK_DATA32) * i, pIat += sizeof(IMAGE_THUNK_DATA32) * i) {
